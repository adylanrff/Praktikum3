...
/* Deklarasi : T : TabInt */
/* Maka cara akses:
   T[i] untuk mkengakses elemen ke-i */
/* Definisi :
...
/* Jika plus = false, mengirimkan T1-T2, yaitu setiap elemen T1 dikurangi elemen T2 pada indeks yang sama */
{
	...
	for (i = GetFirstIdx(T1); IsIdxEff(T1, i); i++) {
		...
		else /* !plus */ Elmt(T3, i) = Elmt(T1, i) - Elmt(T2, i);
	}
        ...
        Min berisi nilai minimum T */
{
	...
	*Min = Elmt(T, i);
	...
	while (IsIdxEff(T, i)) {
		*Max = (*Max < Elmt(T, i))? Elmt(T, i) : *Max;
		*Min = (*Min > Elmt(T, i))? Elmt(T, i) : *Min;
   ...
   algoritma bebas */
{
   ...
   for (i = GetFirstIdx(*T); IsIdxEff(*T, i); i++) {
	   for (j = i+1; IsIdxEff(*T, j); j++) {
		   if ((Elmt(*T, i) < Elmt(*T, j)) ^ asc) {
			   X = Elmt(*T, i);
			   Elmt(*T, i) = Elmt(*T, j);
			   Elmt(*T, j) = X;
		   }
